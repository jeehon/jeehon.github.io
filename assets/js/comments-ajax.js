/*
 * WordPress 內置嵌套評論專用 Ajax comments >> WordPress-jQuery-Ajax-Comments v1.25(no-edit) by Willin Kan.
...
var	ajax_php_url = js_url.replace('js/comments-ajax.js','comments-ajax.php'),				//將 -ajax.js 替換為 -ajax.php, 找到 comments-ajax.php 路徑
		wp_url = js_url.substr(0,js_url.indexOf('/wp-content/')),			//找到 WP 安裝路徑
		pic_sb_url = wp_url + '/wp-content/themes/g-white_2/images/ajax-loader.gif',			//提交 icon 位址
		pic_no_url = wp_url + '/wp-content/themes/g-white_2/images/no.png', 							//錯誤 icon 位址
		pic_ys_url = wp_url + '/wp-content/themes/g-white_2/images/yes.gif', 						//成功 icon 位址
		txt1 = ' style="display: none;background: url(',							//--------------- 以下是過程所用的 html 字段, 儘量不去動它.
		txt2 = ') no-repeat left;padding-left:20px;color:#3B5998;',
		txt3 = '<div id="commentload"'+ txt1 + pic_sb_url + txt2 + '">Submitting your comment, please wait...</div>',
		...
		txt8 = '\n<span id="success_',
		txt9 = '" style="margin-left:15px; background: url(' + txt2 + '"></span>\n',
		txtb, num = 0, $new_comm;
...
$('#commentform').submit(function(){															//id='commentform' submit時的動作
				$('#submit').attr("disabled",true).fadeTo('slow', 0);		//防範再次按提交按鈕
...
var wait = 10, submit_val = $('#submit').val();										//時間設15秒, 暫存:按鈕上的字
